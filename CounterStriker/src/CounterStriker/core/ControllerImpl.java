...
public class ControllerImpl implements Controller {
    ...
    public String report() {
        List<Player> ct = this.players.getModels().stream().filter(p -> p instanceof CounterTerrorist).collect(Collectors.toCollection(ArrayList::new));
        List<Player> t = this.players.getModels().stream().filter(p -> p instanceof Terrorist).collect(Collectors.toCollection(ArrayList::new));

        ct = ct.
                stream()
                .sorted(Comparator
                        .comparing(Player::getHealth)
                        .reversed()
        ...
        for (Player model : ct) {
            stb.append(model).append(System.lineSeparator());
        }
        for (Player model : t) {
            stb.append(model).append(System.lineSeparator());
        }
